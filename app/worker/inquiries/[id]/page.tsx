    ...
    <div className="max-w-2xl">
      ...
      <section className="mt-4 rounded-2xl border border-line bg-white p-6" aria-label="Ubah status">
        ...
        <form action={processWorkerInquiry.bind(null, id)} className="mt-3 flex flex-wrap gap-2">
          ...
          <select id="w-inq-st" name="status" defaultValue={inq.status} className="rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm">
            {STATUSES.map((s) => (
        ...
        <Link href="/worker/inquiries" className="mt-4 inline-block text-sm text-stone-500 hover:text-ink hover:underline">
          ← Kembali ke daftar
        </Link>
      </section>
    </div>
